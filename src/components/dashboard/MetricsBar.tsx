...
interface Metrics {
  ...
  by_sport: Array<{ sport: string; total: number; wins: number; win_rate: number }>
}
    ...
    return (
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 text-center">
        <p className="text-gray-500 text-sm">
          📊 Performance tracking starts once daily picks are resolved. Check back after today's games finish.
        </p>
      </div>
    )
  }

  const winRateColor = metrics.win_rate >= 60 ? 'text-emerald-400' :
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-600">{metrics.total} picks tracked</span>
      </div>

      <div className="grid grid-cols-4 gap-4">
        <div className="text-center">
          <div className="text-2xl font-bold text-white">{metrics.wins}–{metrics.losses}</div>
          <div className="text-xs text-gray-500 mt-1">Record</div>
        </div>
        <div className="text-center">
          <div className={`text-2xl font-bold ${winRateColor}`}>{metrics.win_rate}%</div>
          <div className="text-xs text-gray-500 mt-1">Win Rate</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-blue-300">{metrics.avg_edge_score}%</div>
          <div className="text-xs text-gray-500 mt-1">Avg Edge Score</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-purple-300">{metrics.total}</div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-700 flex gap-4">
          {metrics.by_sport.map(s => (
            <div key={s.sport} className="text-sm">
              ...
              <span className="text-gray-300 ml-2">{s.wins}–{s.total - s.wins} ({s.win_rate}%)</span>
            </div>
          ))}
        </div>
      )}
...
function MetricsSkeleton() {
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 animate-pulse">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[1,2,3,4].map(i => <div key={i} className="h-10 bg-gray-800 rounded"></div>)}
      </div>
    </div>
  )
}