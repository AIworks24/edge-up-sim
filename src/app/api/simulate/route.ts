...
export async function POST(req: NextRequest) {
  ...
  const availableSports = ['ncaab','nba'] 
  ...
  try {
    ...
    const { data: dbRow } = await supabaseAdmin
      .from('sports_events')
      .select('id, neutral_site')
      .eq('external_event_id', String(event_id))
      .single() as any

    const dbNeutralSite = dbRow?.neutral_site ?? false
    const dbEventUUID   = dbRow?.id           ?? event_id

    console.log('[SIMULATE] MSF id:', event_id, '→ UUID:', dbEventUUID)

    const result = await runGameSimulation({
      event_id: dbEventUUID, home_team, away_team,
      home_team_sr_id, away_team_sr_id,
      sport, spread_home, total,
      odds_spread,
      odds_total,
      odds_ml_home: (odds_ml_home != null && (odds_ml_home <= -100 || odds_ml_home >= 100)) ? odds_ml_home : -110,
      odds_ml_away: (odds_ml_away != null && (odds_ml_away <= -100 || odds_ml_away >= 100)) ? odds_ml_away : -110,
